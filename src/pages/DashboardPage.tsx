    ...
    <div className="flex h-dvh flex-col bg-slate-950">
          ...
          <div className="min-w-0">
            ...
            {status?.started && (
              <p className="text-xs break-words text-slate-500">
                Environment: <strong className="text-slate-300">{status.environment}</strong> · Mode:{" "}
                <strong className="text-slate-300">{status.mode}</strong> · Strategy:{" "}
                <strong className="text-slate-300">{status.strategy?.name}</strong>
              </p>
            )}
          </div>
        </div>
      ...
      <div className="flex min-h-0 flex-1">
        <SidebarShell open={sidebarOpen} onClose={() => setSidebarOpen(false)}>
          <Sidebar status={status} onChanged={refresh} />
        </SidebarShell>
